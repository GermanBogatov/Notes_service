...
func (h *Handler) GetNotesByUser(w http.ResponseWriter, r *http.Request) error {
	...
	//TODO VERIFY RSA SIGN dto.SIGN
	h.Logger.Debug("Verify server")
	verifyserver := crypto.VerifySIGN(userUUID, dto.SIGN)
	if verifyserver == true {
		h.Logger.Debug("Corrected Verify!")
	...
	h.Logger.Debug("No corrected Verify!")
	return nil
}
...
func (h *Handler) CreateNote(w http.ResponseWriter, r *http.Request) error {
	...
	h.Logger.Debug("Verify server...")
	verifyserver := crypto.VerifySIGN(dto.Body, dto.SIGN)
	if verifyserver == true {
		h.Logger.Debug("Corrected Verify!")

		dto.UUID = crypto.GenerateKeyUIID()
		h.Logger.Debug("Encrypted body")
...
func (h *Handler) DeleteNote(w http.ResponseWriter, r *http.Request) error {
	...
	h.Logger.Debug("Verify server")
	verifyserver := crypto.VerifySIGN(noteUUID, dto.SIGN)
	if verifyserver == true {
		h.Logger.Debug("Corrected Verify!")
